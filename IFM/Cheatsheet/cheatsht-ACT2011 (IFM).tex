...
Pour un portefeuille à 2 actifs, \lfbox[formula]{$\sigma_{p}^{2}	=	w_{1}^{2}\sigma_{R_{1}}^{2} + w_{2}^{2}\sigma_{R_{2}}^{2} + 2w_{1}w_{2}\rho_{1, 2}\sigma_{R_{1}, R_{2}}$}.\\
...
Le volatilité d'un portefeuille des actifs $A$ et $B$ lorsque 
\begin{description}
	\item[$\rho_{B, W} = 1$]	est maximisée ;
		\begin{itemize}
		\item	La volatilité équivaut à la moyenne pondérée de la volatilité des actifs.
		\end{itemize}
	\item[$\rho_{B, W} < 1$]	est moins que la moyenne pondérée ;
	\item[$\rho_{B, W} = -1$]	un portefeuille \textcolor{amethyst}{sans risque} peut être construit.
\end{description}
...
Donc, on trouve : 
\begin{itemize}
	\item	le coût du capital sans effet de levier \lfbox[formula]{$r_{U}	=	w_{E} \cdot r_{E} + w_{D} \cdot r_{D}$}, et 
...
Pour ce faire, on doit estimer les flux monétaires futurs ainsi que le coût du capital. Cependant ces estimations ont une certaine incertitude et donc nous voyons 4 façons d'évaluer l'impact de cette incertitude et d'identifier les principaux facteurs sous-jacents à la VAN d'un projet.
...
Le coût du capital auquel la VAN est nulle. C'est à dire, le taux auquel la valeur actualisée des rentrées de fonds équivaut la valeur actualisée des sorties de fonds permettant d'équilibrer les comptes.
\end{definitionNOHFILLsub}
...
Il y a 3 facteurs qui influe le calendrier de l'investissement :
\begin{enumerate}
	...
	\item	Les dividendes.
		...
		\item	Il est souvent mieux d'attendre Àà moins qu'il y ait un coût associé.
		\end{itemize}
\end{enumerate}
...
\begin{definitionNOHFILLprop}[\og \textit{Angel Investors} \fg{}]
Typiquement des individus riweches qui sont eux-mêmes des entrepreneurs et qui investissent dans des compagnies pour une portion des actions.
\begin{itemize}
...
\begin{definitionNOHFILLprop}[\og \textit{Venture Capital Firms} \fg{}]
Lorsqu'une entreprise nécessite plus de capitaux que peuvent offrir des \og \textit{angel investors} \fg{}, elle fait appel à un \og \textit{venture capital firm} \fg{}.
\begin{itemize}
	...
	\item	Ces entreprises requièrent habituellement beaucoup de contrôle en échange pour les fonds prêtés.
	\item	Souvent, les commanditaires sont activement impliqués dans la société et font partie du conseil d'administration.
\end{itemize}
\end{definitionNOHFILLprop}

\begin{definitionNOHFILLprop}[\og \textit{Private Equity Firms} \fg{}]
...
\begin{definitionNOHFILLprop}[\og \textit{Institutional Investors} \fg{}]
...
\begin{definitionNOHFILLprop}[\og \textit{Corporate Investors} \fg{}]
...
\begin{itemize}
	...
	\item	Les actions privilégiées se comportent de façon différent selon l'âge de la compagnie.
		\begin{itemize}
		\item	Des \textbf{compagnies adultes} (\og \textit{mature} \fg{}) offrent des dividendes préférentielles, une valeur en cas de liquidation ou des droits de vote avec les actions privilégiées.
...
\begin{itemize}
	\item	On peut trouver le \textbf{pourcentage de participation} d'un investisseur comme $\frac{\text{montant investit}}{\text{évaluation après financement}}$.
...
Les distinctions principales des actions privilégiées que détiennent habituellement les \og venture capitalists \fg{} aux actions ordinaires se résument à :

\begin{definitionNOHFILLprop}[Préférences de liquidité]
...
\begin{itemize}
	\item	La préférence est trouvé par : multiplicateur $\times$ investissement initial
\end{itemize}
\end{definitionNOHFILLprop}

\begin{definitionNOHFILLprop}[Ancienneté]
Ceux qui investissent plus tard peuvent demander une ancienneté plus élevée que les investisseurs des séries antérieures.
\begin{itemize}
	\item	S'ils sont donné une ancienneté égale, ils sont \textit{pari passu} (latin pour "de rang égal").
\end{itemize}
\end{definitionNOHFILLprop}

\begin{definitionNOHFILLprop}[Droits de participation]
...
\begin{definitionNOHFILLprop}[Protection anti-dilution]
...
\begin{itemize}
	\item	Ceci permet d'augmenter le pourcentage de participation.
\end{itemize}
\end{definitionNOHFILLprop}

\begin{definitionNOHFILLprop}[Participation au conseil d'administration]
...
Un investisseur à deux approches possibles :
\begin{description}
	\item[Acquisition]	D'autres investisseurs achètent les parts de l'entreprise.
	\item[Émission publique]	Avec un appel publique à l'épargne (APE).
		\begin{itemize}
		\item	En anglais, \og \textit{inital public offering (IPO)} \fg{}.
		\end{itemize}
\end{description}


\subsubsection{Financement par actions pour des compagnies publiques}
Le première offre publique (financement par action pour une compagnie publique) est la première fois qu'une entreprise vend ses actions au publique. Il y a plusieurs avantages et désavantages.
...
\begin{itemize}
	\item	Détenteurs d'actions dispersés.
		...
		\end{itemize}
	\item	La conformité longue et couteuse.
		\begin{itemize}
...
\begin{definitionNOHFILLprop}[Meilleurs efforts]
...
\begin{itemize}
	...
	\item	Habituellement, ce mécanisme est utilisé pour des petites OP.
\end{itemize}
\end{definitionNOHFILLprop}

\begin{definitionNOHFILLprop}[Vente garantie]
Le souscripteur garanti que toutes les actions seront vendues au cours vendeur.
\begin{itemize}
	\item	Approche la plus courante.
	\item	Les souscripteur achète les actions avec un rabais puis tente de les revendre au cours vendeur.
...
\begin{definitionNOHFILLprop}[Vente aux enchères]
Les actions se vendent à un même prix aux investisseurs les plus offrants tel qu'ils sont tous vendues.
\end{definitionNOHFILLprop}


\paragraph{Mécanisme de l'OP}
\begin{itemize}
	\item	Un groupe de souscripteurs gère l'OP avec un souscripteur principal.
		\begin{itemize}
	...
	\item	La déclaration a 2 composantes principales :
		...
		\item	Prospectus définitif avec des détails sur l'OP (quantité d'actions disponibles, prix, etc.)
		\end{enumerate}
	\item	Une évaluation de la compagnie est effectuée par le souscripteur.
		...
		\item	Habituellement, l'entreprise a une conférence avec les clients les plus importants du souscripteurs pour vendre l'entreprise.
...
\paragraph{Énigmes d'OP}
Quatre aspects de l'OP sont un mystère aux économistes financiers :

\begin{definitionNOHFILLpropos}[Sous-évaluation]
En moyenne, le prix d'émission d'une OP est trop faible.

\begin{itemize}
	\item	En partie, ceci est puisque les souscripteurs souhaitent minimiser leur risque.
	\item	Ceci cause les rendements d'une OP à être anormalement positifs.
...
\begin{itemize}
	\item	Le nombre d'OP n'est pas seulement influencé par le besoin de capitaux propres (alias, en temps de crise).
\end{itemize}
\end{definitionNOHFILLpropos}

\begin{definitionNOHFILLpropos}[Coût d'une OP]
Les coûts de transaction d'une OP sont élevés.
\begin{itemize}
...
En moyenne, la performance à long terme suite à une OP n'est pas bonne.
\end{definitionNOHFILLpropos}
