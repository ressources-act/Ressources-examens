...
La probabilité stationnaire $\pi_{j}$ de l'état $j$ correspond au réciproque de l'espérance du nombre transitions pour qu'une chaîne de Markov ayant débuté dans l'état $j$ y retourne : \lfbox[formula]{$\frac{1}{m_{j}}$}.\\
...
Soit la chaîne de Markov stationnaire \textbf{ergodique} $\{X_{m}, m \geq 0\}$. Alors, le processus inverse est lui-même une chaîne de Markov avec probabilités de transition \lfbox[formula]{$R_{i, j} = \frac{\pi_{j}P_{j, i}}{\pi_{i}}$}.
...
Il s'ensuit qu'elle a la même probabilité d'aller d'un état $i$ à un état $j$ que d'aller d'un état $j$ à un état $i$ peu importe le chemin. C'est à dire, \lfbox[formula]{$P_{i, j}P_{j, k}P_{k, i} = P_{i, k}P_{k, j}P_{j, i}$}
\end{definitionNOHFILLsub}
