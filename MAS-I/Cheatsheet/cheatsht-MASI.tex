...
\end{description}
\end{distributions}
La région critique $\mathcal{C}$ est l'ensemble des valeurs de la statistique que l'on considère trop « extrême » pour être le statu quo et donc que l'on rejette. 

On rejette $\textrm{H}_{0}$ si $\{X_{1}, \dots, X_{n}\} \in \mathcal{C}$ puis on conserve $\textrm{H}_{0}$ si $\{X_{1}, \dots, X_{n}\} \in \mathcal{C}^{c}$.
...
Avec cet approche, on trouve que l'on peut faire 2 types d'erreur, soit une erreur de type I (\og \textit{false positive} \fg{}) ou une erreur de type II (\og \textit{false negative} \fg{}). Le tableau ci-dessous montre ce qu'elles représentent, puis la section sur les \underline{\textit{\ref{subsec:optimTests}}} va plus en détails sur l'optimisation des erreurs.
\end{rappel_enhanced}
...
\draw [line width=1.5]    (428.83,129.33) -- (428.83,134.67) ;
%Straight Lines [id:da041814477790474536] 
\draw [line width=1.5]    (530.83,129.33) -- (530.83,134.67) ;
...
% Text Node
\draw (524.83,132) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle c$};
% Text Node
\draw (414.33,132) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle -c$};
% Text Node
\draw (604.83,124) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle z$};
% Text Node
\draw (481.83,29) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle f( z)$};
\end{tikzpicture}
\end{center}
