...
La variable aléatoire $X$ fait partie de la famille exponentielle si l'on peut récrire sa fonction de probabilité comme : \lfbox[formula]{$f(x) = \textrm{e}^{a(x) \cdot b(\theta) + c(\theta) + d(x)}$} où le domaine de $X$ ne dépend pas du paramètre $\theta$.
\end{definitionNOHFILL}


\lfbox[formula]{$f(x_{1}, \dots, x_{n}) = h_{1}\left(\sum_{i = 1}^{n}a(x_{i}); \theta\right)h_{2}\left(x_{1}, \dots, x_{n}\right)$}
