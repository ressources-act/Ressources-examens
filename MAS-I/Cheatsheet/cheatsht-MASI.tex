...
On décrit un \textbf{intervalle aléatoire} \lfbox[formula]{$(L, U)$} d'un paramètre $\theta$ avec \lfbox[formula]{$\Pr(L \leq \theta \leq U) = k$}. 

\bigskip

Lorsque $L$ et $U$ sont évalués pour les données, nous obtenons un intervalle de confiance \lfbox[formula]{$(l, u)$} de $100k\%$ pour $\theta$ où $k$ est le \textit{\textbf{niveau de confiance}}.
...
\subsection{Intervalles sur la moyenne}
...
\subsection{Intervalles sur les proportions}
\subsubsection{1 échantillon}
