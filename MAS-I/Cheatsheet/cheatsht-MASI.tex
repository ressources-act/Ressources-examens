...
\begin{definitionNOHFILLsub}[\og \textit{Minimal path vector} \fg{}]
\og \textit{Path vector} \fg{} ayant le minimum de composantes pour fonctionner. Donc, le système cesse de fonctionner dès qu'une des composantes qui fonctionne échoue. \\

En termes mathématiques, $\bm{x}$ est un \og \textit{minimal path vector} \fg{} si \lfbox[conditions]{$\phi(\bm{y}) = 0 \forall \bm{y} < \bm{x}$}. 
\begin{itemize}
	\item	$\bm{y} < \bm{x}$ implique que tous les éléments $y_{i}$ du vecteur $\bm{y}$ sont inférieures ou égales aux éléments $x_{i}$ du vecteur $\bm{x}$ ($y_{i} \leq x_{i} \forall i$) avec quelques-uns qui sont strictement inférieurs ($y_{i} < x_{i}$ pour au moins un $i$).
\end{itemize}
\end{definitionNOHFILLsub}
 
\begin{definitionNOHFILLsub}[\og \textit{Minimal path sets} \fg{}]
L'ensemble minimal des composantes dont le fonctionnement garanti le fonctionnement du système. Donc, le système fonctionne uniquement si toutes les composantes d'au moins un des \og \textit{minimal path sets} \fg{} fonctionne.
\end{definitionNOHFILLsub}
