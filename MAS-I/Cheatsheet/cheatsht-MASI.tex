...
\paragraph{Note}	On peut trouver cet estimateur comme l'estimateur non biaisé ayant la plus petite efficacité. Sinon, on peut l'identifier avec le \underline{\textit{\hyperlink{lehSchTh}{théorème de Lehmann-Scheffé}}} ou le théorème de Rao-Blackwell décrits dans la section \underline{\textbf{\nameref{sec:exhStat}}}.
...
\subsubsection{Statistique complète}
...
\begin{enumerate}[label = \circled{\arabic*}{trueblue}]
	...
	\item	Il y existe une fonction unique $\phi(\cdot)$ de $T_{n}$ tel que $\phi(T_{n})$ est une estimateur non biaisé de $\theta$.
\end{enumerate}
alors la statistique $\phi(T_{n})$ est le MVUE de $\theta$.
\end{definitionNOHFILLsub}

\paragraph{Note}	Voir \nameref{subsubsec:MVUECriteria} pour plus de détails sur le MVUE.

\columnbreak
\subsubsection{Statistique exhaustive minimale}\label{subsubsec:exhMinStat}
...
Soit : 
\begin{enumerate}
	\item	la statistique $T_{n}$ exhaustive pour $\theta$, et
	\item	l'estimateur de $\theta$ non biaisé $\hat{\theta}_{n}$
\end{enumerate}
tel que $T_{n}$ n'est pas fonction de $\hat{\theta}_{n}$, et vice-versa. 

\bigskip

Le fait que $T_{n}$ est exhaustif garanti que la distribution de $(\hat{\theta}_{n} | T_{n})$ n'est pas fonction de $\theta$. Il s'ensuit que \lfbox[formula]{$\tilde{\theta}_{n} = \text{E}_{\hat{\theta}_{n}}[\hat{\theta}_{n} | T_{n}]$} est également une statistique non biaisé de $\theta$, puis que \lfbox[formula]{$\text{Var}(\tilde{\theta}_{n}) \leq \text{Var}(\hat{\theta}_{n})$}.
\end{definitionNOHFILLsub}

En bref, le théorème de Rao-Blackwell indique que l'estimateur $\text{E}_{\hat{\theta}_{n}}[\hat{\theta}_{n} | T_{n}]$ a une variance inférieure, ou égale, à tout autre estimateur non biaisé $\hat{\theta}_{n}$; il s'ensuit qu'il remplit tous les critères pour être le MVUE.
