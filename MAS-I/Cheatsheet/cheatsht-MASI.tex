...
Un problème de \textit{\underline{\nameref{sec:regression}}} implique une variable réponse \textbf{quantitative}. 
...
\newpage
\section{Régression linéaire simple et multiple}\label{sec:regression}
...
En traitant la variable réponse comme une variable aléatoire, $b_{0}$ et $b_{1}$ sont des estimateurs et non des estimations. Nous nous intéressons également à la statistique \lfbox[formula]{$\text{E}[Y] = \beta_{0} + \beta_{1} x$} qui est estimé par $\hat{Y}$.

\bigskip

Nous définissons quelques propriétés des estimateurs :

\begin{definitionGENERAL}{Distribution}[\circled{1}{trueblue}]
Les estimateurs $b_{0}$ et $b_{1}$ sont des combinaisons linéaires de la v.a. normale $Y$; ils sont donc normalement distribués. Il s'ensuit que la statistique $\hat{Y}$, fonction de $b_{0}$ et $b_{1}$, sera également normalement distribuée.
\end{definitionGENERAL}

\begin{definitionGENERAL}{Moyenne}[\circled{2}{trueblue}]
Les estimateurs sont sans biais.
\end{definitionGENERAL}

\begin{definitionGENERAL}{Erreur type}[\circled{3}{trueblue}]
variances
$\text{Var}(b_{0}) = \sigma^{2} \left[\frac{1}{n} + \frac{\bar{x}^{2}}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}\right]$
$\text{Var}(b_{1}) = \sigma^{2} \left[\frac{1}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}\right]$
$\text{Var}\left(\hat{Y}\right) = \sigma^{2} \left[\frac{1}{n} + \frac{\left(x - \bar{x}\right)^{2}}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}\right]$

erreurs type estimées si $\sigma^{2}$ n'est pas connu
$\text{SE}(b_{0}) = \sqrt{MSE \left[\frac{1}{n} + \frac{\bar{x}^{2}}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}\right]}$
$\text{SE}(b_{1}) =  \sqrt{\frac{MSE}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}}$
$\text{SE}\left(\hat{Y}\right) = \sqrt{MSE \left[\frac{1}{n} + \frac{\left(x - \bar{x}\right)^{2}}{\sum_{i = 1}^{n} \left(x_{i} - \bar{x}\right)^{2}}\right]}$
\end{definitionGENERAL}



$\bm{b} = \left(\bm{X}^{\top} \bm{X}\right)^{-1}\bm{X}^{\top}\bm{Y}$

donc matrice de variance-covariance de $\bm{b}$ est $\text{Var}(\bm{b}) = \sigma^{2} \left(\bm{X}^{\top} \bm{X}\right)^{-1}$ où 
\begin{align*}
	\text{Var}(\bm{b})
	&=	\begin{bmatrix}
...
Si $\sigma^{2}$ est inconnu, $\widehat{\text{Var}}(\bm{b}) = MMSE \left(\bm{X}^{\top} \bm{X}\right)^{-1}$ où 
\begin{align*}
	...
	&=	\begin{bmatrix}
		...
		\end{bmatrix},
\end{align*}
où $\widehat{\text{Var}}(b_{0}) =\text{se}(b_{0})^{2}$ et $\widehat{\text{Var}}(b_{1}) =\text{se}(b_{1})^{2}$.
...
\begin{align*}
	...
	\mathrm{H}_{1}&: \beta_{1} > 0
\end{align*}

$t = \frac{b_{1} - h}{se(b_{1})}$

\columnbreak
\subsubsection{Intervalle de confiance et de prévision}
confiance
	$\beta_{0}$
	$\beta_{1}$
	$\text{E}[Y]$
...
\columnbreak
\subsection{Régression linéaire multiple}\label{subsec:MLR}



\newpage
\section{Régression linéaire généralisée}
