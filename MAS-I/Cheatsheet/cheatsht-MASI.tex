...
\newpage	
\section{Simulation}
Appel les variables aléatoires indépendantes simulées d'une distribution les \textit{nombres aléatoires}

\subsection{Uniform number generation}
méthode
	conventionnelle
	computationelle	
\begin{algo2}[Simulation d'une variable aléatoire uniforme]
Pour une variable aléatoire $U(0, 1)$, 
\begin{enumerate}[label = \circled{\arabic*}{trueblue}]
	\item	Choisir l'ancrage : un nombre initial $x_{0}$.
	\item	Générer des nombres pseudo-aléatoires avec \lfbox[formula]{$x_{j + 1} = (ax_{j} + c)\mod m$}, \lfbox[conditions]{$n \geq 0$}.
		\begin{itemize}
		\item	Les valeurs $a, c, m$ sont spécifiées en avance pour imiter une simulation aléatoire.
		\end{itemize}
...
\begin{enumerate}[label = \circled{\arabic*}{trueblue}]
	...
	\item	Simuler une réalisation $y_{j}$ de la variable aléatoire $Y$ ayant la fonction de densité $g$.
