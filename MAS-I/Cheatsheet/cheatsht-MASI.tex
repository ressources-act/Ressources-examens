...
\begin{rappel_enhanced}[Contexte]
La méthode des moindres carrés (\og \textit{ordinary least squares (OLS)} \fg{}) estime les coefficients de régression en minimisant la \lfbox[formula]{$\displaystyle SSE = \sum_{i = 1}^{n} \left(y_{i} - b_{0} - b_{1}x_{i, 1} - \dots - b_{p} x_{i, p}\right)^{2}$}. Par le \textit{\underline{\nameref{subsubsec:BVtradeoff}}}, augmenter la $p$ (la flexibilité) diminue le biais de la méthode d'OLS mais augmente sa variance.
