...
\begin{definitionNOHFILL}[Régression linéaire simple]
...
On obtient les prévisions $\hat{y}$ en fonction des estimations des paramètres libres avec \lfbox[formula]{$\hat{y} = b_{0} + b_{1} x$}.

\bigskip

Avec les données d'entraînement, on estime les paramètres libres par \textit{\textbf{la méthode des moindres carrés ordinaires}}. On minimise la somme des différences entre les valeurs observées et prédites de la variable réponse : $\sum_{i = 1}^{n} \left(y_{i} - \hat{y}_{i}\right)^{2} = \sum_{i = 1}^{n} \left(y_{i} - b_{0} - b_{1} x_{i}\right)^{2}$ afin d'obtenir les estimations \lfbox[formula]{$\displaystyle b_{1} = \frac{\sum_{i = 1}^{n} (x_{i} - \bar{x})(y_{i} - \bar{y})}{\sum_{i = 1}^{n} (x_{i} - \bar{x})^{2}}$} et \lfbox[formula]{$b_{0} = \bar{y} - b_{1} \bar{x}$}.
...
La variance de la valeur réponse, qui correspond à la variance de l'erreur de prévision, est estimée par l'erreur quadratique moyenne de prévision : \lfbox[formula]{$\displaystyle MSE = \frac{\sum_{i = 1}^{n} \left(y_{i} - \hat{y}_{i}\right)^{2}}{n - 2}$}. 
	
\paragraph{Note}	On divise par $n - 2$ puisque les paramètres $\beta_{0}$ et $\beta_{1}$ sont estimés. Le MSE de la section de \textit{\underline{\nameref{subsec:PrecisionModel}}} estime une mesure de la précision du modèle alors qu'ici on estime sans-biais la précision. Donc, il n'était pas nécessaire de diviser par $n - 2$ au lieu de $n$.
