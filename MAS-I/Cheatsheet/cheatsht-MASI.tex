...
\begin{rappel_enhanced}[Contexte]
Parfois il peut ne pas être nécessaire de trouver la fonction de fiabilité exacte. En lieu, on peut  l'approximer en trouvant les bornes supérieures et inférieures de la fonction avec une des deux méthodes suivantes.
\end{rappel_enhanced}
...
\begin{align*}
	...
	&=	\sum_{j = 1}^{n} \Pr(E_{j}) -
		\sum_{j = 1}^{n} \sum_{k > j} \Pr(E_{j} \cap E_{k}) + \\
&\quad		\sum_{j = 1}\sum_{k > j}\sum_{l > k} \Pr(E_{j} \cap E_{k} \cap E_{l}) - 
		\dots + (-1)^{n + 1}	\\
\end{align*}
\end{rappel}
...
\begin{align*}
	&\underbrace{\prod_{j = 1}^{m} \left[1 - \prod_{i \in C_{j}} (1 - p_{i}) \right]}_{\substack{\text{probabilité qu'au moins une composante de}\\	\text{chacun des \og \textit{minimal cut sets} \fg{} fonctionne}}}
	&\leq	r(\bm{p})	&\leq	
	&\underbrace{1 - \prod_{j = 1}^{s} \left[1 - \prod_{i \in A_{j}} p_{i} \right]}_{\substack{\text{probabilité que toutes les composante}\\	\text{d'au moins un des \og \textit{minimal path sets} \fg{}}\\ \text{fonctionnent}}}
\end{align*}
