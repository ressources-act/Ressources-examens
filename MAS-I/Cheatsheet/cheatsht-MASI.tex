...
\begin{definitionNOHFILL}[Résidus $e_{i}$]
...
\begin{definitionNOHFILL}[Coefficient de détermination $R^{2}$]
\begin{rappel_enhanced}[Contexte]
...
\begin{definitionNOHFILLsub}[Résidus standardisés]
...
\begin{definitionNOHFILLsub}[Résidus studentisés]
...
Les fonctions de lien doivent être \textbf{monotones} et \textbf{différentiables}, en voici quelques-unes où  \lfbox[formula]{$\mu  = g^{-1}(\matr{x^{\top}}\bm{\beta})$}  : 
...
\subsubsection{Méthode de \og \textit{Scoring} \fg{}}
\begin{rappel_enhanced}[Contexte]
...
Déviance \lfbox[formula]{$D = 2\left(\ell_{sat} - \ell(\hat{\bm{b}})\right)$}

Pseudo $R^{2}$ \lfbox[formula]{$R^{2}_{pse.} = \frac{\ell_{null} -  \ell(\hat{\bm{b}})}{\ell_{null}}$}.

Critères d'information
\lfbox[formula]{$AIC = -2  \ell(\hat{\bm{b}}) + 2 (\text{nomobrme de paramètres estimés})$}
\lfbox[formula]{$BIC = -2  \ell(\hat{\bm{b}}) + \ln(n) (\text{nomobrme de paramètres estimés})$}


\subsubsection{Résidus}
raw residual $e_{i} = y_{i} - \hat{\mu}_{i}$


\begin{definitionNOHFILL}[Résidus de Pearson]
\lfbox[formula]{$e_{i}^{P} = \frac{e_{i}}{\sqrt{\widehat{\text{Var}}(Y_{i})}}$}.
\lfbox[formula]{$e_{sta., i}^{P} = \frac{e_{i}^{P}}{\sqrt{1 - h_{i}}}$}.

Statistique du khi carré de Pearson \lfbox[formula]{$\sum_{i = 1}^{n} \left(e_{i}^{p}\right)^{2}$}
\end{definitionNOHFILL}

\begin{definitionNOHFILL}[Résidus de déviance]
\lfbox[formula]{$e_{i}^{D} = \pm \sqrt{D_{i}}$}.
\lfbox[formula]{$e_{sta., i}^{D} = \frac{e_{i}^{D}}{\sqrt{1 - h_{i}}}$}.
\end{definitionNOHFILL}
