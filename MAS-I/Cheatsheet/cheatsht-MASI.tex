...
Lorsque la variance de l'échantillon aléatoire est connue, on applique le théorème centrale limite. Dans le cas où la variance est inconnue, nous avons recours à \textit{\underline{\hyperlink{studentReminder}{la loi de Student}}}. 
...
%\columnbreak
\subsection{Tests sur les proportions}
...
Loi du khi carrée
	relation loi normale
%	statistique (équivalent TCL)
	propriétés
		non négative
		asymétrique vers la droite

Loi de Fisher
	relation khi carrées
%	statistique
	relation inverse
		propriétés
			non négative
			asymétrique vers la droite, mais pas autant (penser relation avec l'inverse)


\subsubsection{1 échantillon}
Pour un échantillon aléatoire de taille $n$ tiré d'une \textbf{distribution normale} de variance $\sigma^{2}$, la loi du khi carrée implique que \lfbox[formula]{$T_{n} = \frac{(n - 1)S^{2}_{n}}{\sigma^{2}} \sim \chi_{(n - 1)}^{2}$}. Il s'ensuit que \lfbox[formula]{$\frac{(n - 1)s_{n}^{2}}{h}$}.
...
\paragraph{Note}	Puisque la distribution khi carrée est asymétrique, on ne peut pas simplifier d'avantage les régions critique avec des valeurs absolues comme avec la loi normale.
...
Pour 2 échantillons aléatoires indépendants de tailles $n_{1}$ et $n_{2}$ qui sont normalement distribués avec variances $\sigma^{2}_{1}$ et $\sigma^{2}_{2}$, la loi de Fisher implique que \lfbox[formula]{$T_{n} = \frac{S^{2}_{1}/\sigma^{2}_{1}}{S^{2}_{2}/\sigma^{2}_{2}} \sim F_{(n_{1} - 1, n_{2} - 1)}$}. Il s'ensuit que \lfbox[formula]{$\frac{s^{2}_{1}}{s^{2}_{2}} \times \frac{1}{h}$}.

\bigskip

Les régions critiques sont et hypothèses alternatives sont : 
\begin{center}
\begin{tabular}{| >{\columncolor{beaublue}\centering}m{1.5cm} | >{\columncolor{beaublue}}c  | >{\columncolor{beaublue}}c  | >{\columncolor{beaublue}}c	|}
\hline\rowcolor{airforceblue} 
\textcolor{white}{\textbf{}}	&	\textcolor{white}{\textbf{unilatéral à gauche}}	&	\textcolor{white}{\textbf{bilatéral}}	&	\textcolor{white}{\textbf{unilatéral à droite}}		\tabularnewline\specialrule{0.1em}{0em}{0em} 
$\textrm{H}_{0}$	&	$\frac{\sigma^{2}_{1}}{\sigma^{2}_{2}} < h$		&	$\frac{\sigma^{2}_{1}}{\sigma^{2}_{2}} < h$	&	$\frac{\sigma^{2}_{1}}{\sigma^{2}_{2}} \neq h$		\tabularnewline\hline
$\mathcal{C}$	&	$t \leq F_{1 - \alpha, n_{1} - 1, n_{2} - 1}$		&	$\left\{t \leq \left(F_{\alpha/2, n_{2} - 1, n_{1} - 1}\right)^{-1}\right\} \bigcup \left\{t \leq F_{\alpha/2, n_{1} - 1, n_{2} - 1}\right\}$	&	$t \geq F_{\alpha, n_{1} - 1, n_{2} - 1}$		\tabularnewline\hline
\end{tabular}
\end{center}
...
probabilités de survie.
	seule avec cette particularité


\columnbreak
\subsection{Puissance d'un test}\label{subsec:PowerTests}
\begin{definitionNOHFILL}[La puissance d'un test]
La probabilité de \textit{correctement} rejeter l'hypothèse nulle : \lfbox[formula]{$\Pr((X_{1}, \dots, X_{n}) \in \mathcal{C} | \theta \in \Theta_{0})$}
...
Ceci généralise la puissance en posant que la \textit{vraie} hypothèse \underline{peut} être l'alternative et donc, la fonction de puissance est une fonction de $\theta$. 
...
Par exemple, si on rejette l'hypothèse nulle alors on pourrait tracer la fonction de puissance pour toutes les valeurs possibles de l'ensemble $\Theta_{1}$.
...
\columnbreak
\subsection{Tests optimaux}\label{subsec:optimTests}
\begin{distributions}[Notation]
\begin{description}
