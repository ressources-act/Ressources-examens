...
L'erreur quadratique moyenne est définie comme \lfbox[formula]{$\text{MSE}_{\hat\theta}(\theta) = \text{E}[(\hat{\theta}_{n} - \theta)^{2}]$}. Également, on peut réécrire l'expression comme \lfbox[formula]{$\text{MSE}_{\hat\theta}(\theta) = \text{Var}(\hat{\theta}_{n}) + \left[\text{B}(\hat{\theta}_{n})\right]^{2}$}.
...
\begin{rappel_enhanced}[Motivation]
Nous voulons alors une mesure qui n'indique pas seulement qu'un estimateur arrive près de la bonne valeur souvent \textit{(alias, une très petite variance)}, mais qu'il est mieux que d'autres estimateurs. Alors, un autre aspect à évaluer d'un estimateur est sa convergence pour de grands échantillons. \\
...
\begin{rappel_enhanced}[Limitations]
Dût à la sélection arbitraire de l'erreur $\varepsilon$, il est possible d'être sournois avec le choix de $\varepsilon$.
\end{rappel_enhanced}
...
On peut penser à l'information de Fisher comme une mesure de la sensibilité de la log-vraisemblance $\ell(\theta)$ aux données. Une information élevée, exprimée par une variabilité de $\ell'(\theta)$ élevée, suggère que la forme de $\ell(\theta)$ est sensible aux données.
\end{rappel_enhanced}
...
Dans le cas multivarié, \lfbox[formula]{$\text{Var}(\hat{\theta_{j}}) \geq \bm{I}^{-1}(\bm{\theta})_{j, j}$}.
\end{definitionNOHFILL}
...
La fonction de vraisemblance \textbf{est concave} et, puisque sa première dérivée est nulle à $\hat{\theta}^{\text{EMV}}_{n}$, elle va augmenter avant ce point puis diminuer par après. \lfbox[imphl]{La première dérivée permet donc de trouver une fonction qui est maximisée à $\hat{\theta}^{\text{EMV}}_{n}$}. Cependant, ceci ne permet pas d'identifier une fonction unique---plusieurs fonctions peuvent être maximisées au même \textbf{point} tout en ayant des formes différentes.	\\
...
L'\og \textit{efficiency} \fg{} d'un estimateur $\hat{\theta}_{n}$ est définie comme \lfbox[formula]{$\text{eff}(\hat{\theta}_{n}) = \frac{1}{\bm{I}(\theta)\text{Var}(\hat{\theta})}$}.

\begin{definitionNOHFILLsub}[Estimateur \og \textit{efficient} \fg{}]
Si \lfbox[formula]{$\text{eff}(\hat{\theta}_{n}) = 1$}, alias la variance de l'estimateur est égale à la borne Cramér-Rao, l'estimateur \textbf{est \og \textit{efficient} \fg{}}.
\end{definitionNOHFILLsub}
\end{definitionNOHFILL}
...
%%	De efficiency.
%Étant égal à la borne, il \textit{doit} être l'estimateur avec la plus petite de tous les estimateurs sans biais. On dit qu'il est le \og \textbf{\textit{Minimum Variance Unbiased Estimator (MVUE)}} \fg{}.
...
\begin{definitionNOHFILLsub}[Convergence en distribution de l'EMV]
...
\paragraph{Note}	Souvent ces conditions ne sont pas vues en classe puisqu'elles sont compliquées. Alors, ne vous en faites pas si vous ne les comprenez pas complètement.
...
Lorsqu'une distributions a plusieurs paramètres, $\theta$ ne sera plus un scalaire (un seul paramètre) mais plutôt un vecteur. Pour le cas multivarié où une distribution a $r$ paramètres, on dénote le vecteur $\bm{\theta}	=	(\theta_{1}, \cdots, \theta_{r})^{\top}$.\\


\begin{definitionNOHFILLprop}[Matrice d'information de Fisher]
\begin{rappel_enhanced}[Contexte]
Dans la sous-section sur la \underline{\textit{\nameref{sec:cramer_rao}}}, nous avons utilisé la "matrice" d'information de Fisher pour un distribution ayant un paramètre $\theta$. Le calcul de la matrice est le même pour plusieurs paramètres, sauf que l'on représente $\theta^{2}$ comme $\bm{\theta} \bm{\theta}^{\top}$.
\end{rappel_enhanced}
...
\end{align*}
\begin{itemize}
	\item	Cette définition est pour une seule observation $x$, pour plusieurs observations l'échantillon de données sera typiquement un échantillon aléatoire et donc $\bm{I}_{n}(\bm{\theta})	=	n\bm{I}(\bm{\theta})$.
	\item	On dénote l'inverse de la matrice d'information de Fisher par $\bm{I}^{-1}_{n}(\bm{\theta})$.
\end{itemize}
\end{definitionNOHFILLprop}
...
Sous \hyperlink{reg_cond}{\color{blue!40!green!80!black}certaines conditions de régularité}, \lfbox[formula]{$\text{Var}(\tilde{\bm{\theta}})	=	\bm{I}^{-1}_{n}(\bm{\theta})$} et \lfbox[formula]{$\sqrt{n}\left( \hat{\bm{\theta}}	-	\bm{\theta} \right) \overset{D}{\rightarrow} \mathcal{N}_{k}\left( 0, \bm{I}^{-1}_{n}(\bm{\theta}) \right)$}.
