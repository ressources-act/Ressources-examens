...
\begin{definitionNOHFILL}[Matrice de projection $\bm{H}$]
...
\lfbox[formula]{$h_{i} = \frac{1}{n} + \frac{(x_{i} - \bar{x})^{2}}{\sum_{j = 1}^{n} (x_{j} - \bar{x})^{2}}$}.

\bigskip

Règle du pouce c'est que la $i^{\text{e}}$ observation est un \og \textit{high leverage point} \fg{} si \lfbox[conditions]{$h >  3\left(\frac{p'}{n}\right)$}.
\end{definitionNOHFILL}

\begin{definitionNOHFILLsub}[Résidus standardisés]
\lfbox[formula]{$e_{sta, i} = \frac{e_{i}}{\sqrt{MSE(1 - h_{i})}}$}
\end{definitionNOHFILLsub}

\begin{definitionNOHFILLsub}[Résidus studentisés]
\lfbox[formula]{$e_{stu, i} = \frac{e_{i}}{\sqrt{MSE_{(i)}(1 - h_{i})}}$}
\end{definitionNOHFILLsub}
Leverage
Residuals
...
DFITS : \lfbox[formula]{$DFITS = e_{sta, i} \sqrt{\frac{h_{i}}{1 - h_{i}}}$}.
\end{definitionNOHFILL}

\begin{definitionNOHFILL}[\og \textit{Cooks's distance} \fg{}]
\og \textit{\textbf{Cook's distance}} \fg{} : \lfbox[formula]{$d_{i} =  \frac{DFITS^{2}_{i}}{p'}$}. \lfbox[formula]{$d_{i} = \frac{e^{2}_{sta, i}  h_{i}}{p'(1 - h_{i})}$}. \lfbox[formula]{$d_{i} = \frac{e^{2}_{i} h_{i}}{MSE p' (1 - h_{i})^{2}}$}.

\bigskip

Règle du pouce c'est que la $i^{\text{e}}$ observation est un \og \textit{influential point} \fg{} si $d_{i}$ \og \textit{exceeds \textbf{unity}} \fg{}. C'est-à-dire, si \lfbox[conditions]{$d_{i} > 1$}.
\end{definitionNOHFILL}
