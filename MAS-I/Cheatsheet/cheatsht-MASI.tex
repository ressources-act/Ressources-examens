...
La statistique $T_{n}$ est une \underline{\textbf{statistique exhaustive}} pour $\theta$ ssi \lfbox[formula]{$f(x_{1}, \dots, x_{n} | t) = h(x_{1}, \dots, x_{n})$} où la fonction $h(\cdot)$ ne dépend pas de $\theta$.	\\

C'est-à-dire que la distribution de l'échantillon, conditionnelle à la valeur $t$ de la statistique $T_{n}$, suit une distribution qui ne dépend pas de $\theta$. Alors, savoir que $T_{n} = t$ nous donne \textit{\textbf{suffisamment}} d'information à propos de l'effet de $\theta$ sur l'échantillon sans avoir à connaître les $n$ valeurs observées.
\end{definitionNOHFILL}
...
\begin{definitionNOHFILLsub}[Théorème de factorisation de Fisher-Neymann]
La statistique $T_{n}$ est une \underline{\textbf{statistique exhaustive}} pour $\theta$ si \lfbox[formula]{${\color{burntorange}f(x_{1}; \theta) \times \hdots \times f(x_{n}; \theta)} = h_{1}({\color{teal}t}; {\color{amethyst}\theta}) \times h_{2}(x_{1}, \dots, x_{n})$} où 
...
\columnbreak
\subsubsection{Statistique exhaustive minimale}
