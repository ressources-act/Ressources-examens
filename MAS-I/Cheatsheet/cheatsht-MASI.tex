...
On obtient que \lfbox[formula]{$SSR = \sum_{i = 1}^{n} \left(\hat{y}_{i} - \bar{y}\right)^{2}$}.
\end{definitionGENERAL}
...
Le coefficient de détermination \lfbox[formula]{$R^{2} = \frac{SSR}{SST} = 1 -  \frac{SSE}{SST}$}. Également, \lfbox[formula]{$R^{2} = b_{1}^{2} \sum_{i = 1}^{n}(x_{i} - \bar{x})^{2}$}.
\end{definitionNOHFILL}
