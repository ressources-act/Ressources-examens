...
Donc, tout système peut être traité comme le système en parallèle de ses \og \textit{minimal path sets} \fg{}.
...
Donc, tout système peut être traité comme le système en série de ses \og \textit{minimal cut sets} \fg{}.
...
\begin{description}
	\item[$X_{i}$]	Variable aléatoire suivant une distribution Bernoulli : \lfbox[conditions]{$X_{i} \sim \text{Bernoulli}(p_{i})$}.
		\begin{itemize}
		\item	$\bm{X}$ vecteur des v.a. Bernoulli.
		\end{itemize}
	\item[$p_{i}$]	Fiabilité de la $i^{\text{e}}$ composante.
		\begin{itemize}
		\item	$\bm{p}$ vecteur des fiabilités.
		\end{itemize}
\end{description}
\end{distributions}
...
\lfbox[formula]{$r(\bm{p}) = \Pr(\phi(\bm{X}) = 1)$}.
