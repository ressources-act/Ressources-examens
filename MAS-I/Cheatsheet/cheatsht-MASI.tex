...
\begin{align*}
	\bm{I}(\theta) 
	&=	\text{E}\left[\Big(\deriv{\theta}{} \ln f(x; \theta)\Big)^{2}\right]	
	\overset{\text{iid}}{=}	\text{E}\left[{\color{red}\shortminus}\deriv[2]{\theta}{\ln f(x; \theta)}	\right]	\\
	\bm{I}_{n}(\theta) 
...
\begin{rappel_enhanced}[Contexte]
En lieu de supposer une distribution continue pour $f(x)$ puis d'estimer ses paramètres, on peut choisir de \underline{directement} estimer la fonction de densité avec un \textbf{estimateur à noyau de la densité}.
\end{rappel_enhanced}

\begin{definitionNOHFILL}[Fonction noyau $k()$]
La fonction noyau $k()$ est une \textit{fonction de densité} à deux paramètres ($x_{i}$ et $b$).\\

\begin{rappel_enhanced}[Contexte]
Les fonctions noyaux faisant partie de l'examen sont \textbf{symétriques} avec $x_{i}$ comme point milieu. \\

On dénote donc la fonction \lfbox[formula]{$k_{i}()$} comme la fonction noyau dont le domaine est centrée autour de l'observation $x_{i}$.
\end{rappel_enhanced}
\end{definitionNOHFILL}
...
L'interprétation de la largeur de la bande $b$ varie selon la fonction noyau.
...
\begin{enumerate}[label = \circled{\arabic*}{trueblue}]
	...
	\item	Estimer la fonction de densité $f(x)$ comme la moyenne de $k_{1}(x), \dots, k_{n}(x)$ : 
		\begin{align*}
		\tilde{f}(x)
...
\subsection{Noyau rectangulaire (uniforme)}
