...
\begin{enumerate}[label	=	\circled{\arabic*}{trueblue}]
	...
	\item	Calculer le déterminant de la matrice Jacobienne $J$ : 
		...
		\end{bmatrix}
		\end{align*}
	\item	Trouver la fonction de densité conjointe avec \lfbox[formula]{$f_{W_{1}, \dots, W_{n}}(w_{1}, \dots, w_{n})	=	f_{X_{1}, \dots, X_{n}}\left(g^{-1}_{1}(w_{1}, \dots, w_{n}), \dots, g^{-1}_{n}(w_{1}, \dots, w_{n})\right) |J|$}.
\end{enumerate}
...
\columnbreak
\subsection{Mesures de risque et queues de distributions}
...
On peut comparer les queues de distributions en évaluant:
\begin{definitionGENERAL}{Nombre de moments (positifs) qui existent}[\circled{1}{trueblue}]
\textit{Plus} la queue est \textbf{\textbf{lourde}}, \textit{moins} il est probable que l'intégrale de $x^{k}f(x)$ va converger.
\end{definitionGENERAL}

\begin{definitionGENERAL}{Ratio des fonctions de survie (ou PDF)}[\circled{2}{trueblue}]
\textit{Plus} la queue est \textbf{\textbf{légère}}, \textit{plus} la fonction de survie va tendre vers 0 rapidement.\\
...
Si la fonction de hasard est \textit{décroissante}, il y a une probabilité plus élevée de pertes extrêmes et donc une queue \textit{\textbf{lourde}}.
\end{definitionGENERAL}

\begin{definitionGENERAL}{CTEs (ou quantiles)}[\circled{4}{trueblue}]
Plus le CTE est large, plus les montants de pertes extrêmes sont larges et donc plus la queue est \textit{\textbf{lourde}}.
\end{definitionGENERAL}
