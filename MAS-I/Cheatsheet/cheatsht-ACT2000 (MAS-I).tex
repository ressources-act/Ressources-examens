...
L'utilisation des statistiques pour estimer les relations entre des variables. \\
L'apprentissage statistique estime une relation fonctionnelle entre des variables explicatives et un résultat.
\end{definitionNOHFILL}
...
\begin{description}
	\item[prévision]	Prévoir la valeur de la variable réponse pour des valeurs spécifiques des variables réponse.
	\item[inférence]	Comprendre quelles variables explicatives sont liées à la variable réponse et le degré de leurs relations.
\end{description}

\begin{definitionNOHFILLprop}[Flexibilité du modèle]
Les modèles varient en \og flexibilité \fg{}. C'est-à-dire que certains modèles s'ajustent mieux aux données.\\

Par exemple, une régression linéaire correspond à une ligne droite. En réalité, il est peut probable que les données soient situées sur une droite. En contraste, une \og \textit{spline} \fg{} va passer à travers tous les points.\\

Il y a cependant un compromis entre la flexibilité d'un modèle et sa facilité d'interprétation :
\begin{itemize}
	\item	Les modèles \textit{moins flexibles} sont plus facilement interprétables au coût de moins bonnes prévisions. Ils sont généralement mieux pour l'inférence. 
	\item	Les modèles \textit{plus flexibles} plus difficilement interprétables au bénéfice de meilleures prévisions. Ils sont généralement mieux pour faire des prévisions.
\end{itemize}

Cependant, si un modèle est sur ajusté aux données (e.g., un \og \textit{spline} \fg{}) il pourrait avoir des mauvaises prévisions pour des nouvelles données.
\end{definitionNOHFILLprop}

Il y a 2 types d'apprentissage statistique : 
\begin{description}
...
Il y existe plusieurs types de variables :
\begin{definitionNOHFILLpropos}[Variable continue]
Définie sur les nombres réels.	\\

Par exemple :
\begin{itemize}
	\item	Le montant de perte.
	\item	Le temps avant qu'une réclamation soit réglé.
\end{itemize}
\end{definitionNOHFILLpropos}

\begin{definitionNOHFILLpropos}[Variable catégorielle]
Définie sur un petit nombre de valeurs catégorielles.	\\

Par exemple : 
\begin{itemize}
	\item	Une variable binaire. 
		\begin{itemize}
		\item	P. ex., une variable "Maison a un système d'alarme" prenant comme valeur "oui" ou "non".
		\item	P. ex., le sexe d'un individu prenant comme valeur "homme" ou "femme".
		\end{itemize}
	\item	Une variable avec plusieurs niveaux.
		\begin{itemize}
		\item	P. ex., la marque d'une voiture assuré prenant comme valeurs "Toyota", "Honda", etc.
		\end{itemize}
\end{itemize}
...
\begin{description}
	\item[nominale]	Il n'y a pas d'ordre aux catégories.
		\begin{itemize}
		\item	P. ex., le programme d'étude d'un étudiant "actuariat", "comptabilité", etc.
		\end{itemize}
	\item[ordinale]	Il y a une d'ordre aux catégories.
		\begin{itemize}
		\item	P. ex., la sévérité d'un incendie allant de 1 à 5.
		\end{itemize}
\end{description}
...
\begin{itemize}
	\item	On dit aussi \textit{variable qualitative}.
\end{itemize}
\end{definitionNOHFILLpropos}
