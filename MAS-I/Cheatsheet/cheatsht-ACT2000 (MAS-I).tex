...
Les tests de Kolmogorov-Smirnov et de Anderson-Darling sont idéales lorsque l'on désire comparer les fonctions de répartition.
...
On teste si les données supportent l'hypothèse nulle avec la statistique de Kolmogorov-Smirnov : \lfbox[formula]{$D	=	\underset{x_{(1)}	\leq	x	\leq	x_{(n)}}{\max} \left| \hat{F}(x)	-	F^{\ast}(x) \right|$}.
\begin{itemize}
...
\begin{align*}
	A^{2}
	&=	-\frac{1}{n} \left[
		\sum_{j = 1}^{n} (2j - 1)\{\log(F^{\ast}(x_{(j)}) + \log(1	-	F^{\ast}(x_{(n + 1 - j)})\}	-	n
	\right]
\end{align*}

\begin{itemize}
	\item	Comme le test de Kolmogorov-Smirnov, des valeurs critiques sont spécifiées pour une distribution entièrement spécifiée.
\end{itemize}
\end{definitionNOHFILLsub}
...
\begin{definitionNOHFILLsub}[Test du rapport de vraisemblancew]
