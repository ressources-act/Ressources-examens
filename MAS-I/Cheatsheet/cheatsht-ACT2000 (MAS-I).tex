...
On peut observer que $X_{(k)}	\sim	Beta(\alpha	=	k, \beta	=	n - k + 1)$
\end{algo}

Nous pouvons également définir quelques autres statistiques d'intérêt :

\begin{description}
	\item[$R = X_{(n)} - X_{(1)}$: ] \textbf{L'étendue} (range) est la différence entre le minimum et le maximum d'un échantillon.
		\begin{itemize}[leftmargin = *]
...
Pour $n$ données observées, la $k^{\text{e}}$ statistique d'ordre $Y_{(k)}$ est le $\frac{k}{(n + 1)}^{\text{e}}$ quantile.
...
Ces chiffres forment d'ailleurs le diagramme de boîte à moustaches (\og \textit{boxplot} \fg{}) avec \lfbox[conditions]{$h = 1.5 \cdot (Q_{3} - Q_{1})$} : 
\begin{center}
\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
\begin{itemize}
	\item	Il s'ensuit que l'on connait la distribution, mais pas les paramètres ;
	\item	Dans le cas d'une loi normale, on peut centrer et réduire pour obtenir $Z$ ce qui correspond donc à une q-q plot \textbf{normale} ;
	\item	\hl{Autrement, on estime les paramètres avec l'échantillon} (\underline{vérifier}).
\end{itemize}
...
\subsection{Tests de mauvaise classification et diagnostiques}
Test de Kolmogorov-Smirnov
Test de Anderson-Darling
Test d'adéquation du khi-carré
Test du rapport de vraisemblance

\subsection{Critère d'information pour la sélection de modèles}
AIC
BIC
