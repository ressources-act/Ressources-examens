...
\begin{enumerate}
	...
	\item	les accroissement qui ne se \textbf{chevauchent pas} sont indépendantes.
\end{enumerate}
...
\begin{itemize}
	\item	On pose donc que le paramètre de la fréquence des accroissement $\lambda$ est la \textit{moyenne} de la fonction d'intensité des accroissements $\lambda(t)$ sur l'intervalle de temps $(t, t + h]$.
\end{itemize}
...
\begin{enumerate}[label	=	\circled{\arabic*}{trueblue}]
	\item	Les intervalles qui ne se chevauchent pas sont indépendantes.
...
\begin{center}
{\footnotesize temps d'occurences}\\{\footnotesize \textbf{\textcolor[rgb]{0.29,0.56,0.89}{ordonnés}}}
\end{center}
...
\begin{center}
{\footnotesize temps d'occurences}\\{\footnotesize \textbf{\textcolor[rgb]{0.96,0.65,0.14}{non ordonnés}}}
\end{center}
...
\begin{enumerate}[label = \rectangled{\arabic*}{lightgray}]
	\item	On connait l'intervalle de temps durant laquelle les 3 premiers autobus arrivent. \\
...
\begin{enumerate}[label = \rectangled{\arabic*}{lightgray}]
	\setcounter{enumi}{1}
	\item	La \og \textit{twist} \fg{} pour calculer la probabilité est de la voir comme une binomiale.
	\item	D'abord, puisque $X_{k}	\sim \text{U}(8, 9)$ alors la probabilité que n'importe quel des autobus arrivent dans la première demi-heure est $\Pr(X_{k} \leq 0.5) = \frac{1}{9 - 8 + 1} = 0.50$ pour $k = 1, 2, 3$.
...
Si un processus de Poisson peut être décomposé en plusieurs sous-processus distincts, alors ces sous-processus distincts sont également des processus de Poisson avec une intensité proportionnelle. Ce processus de décomposition s'appelle le \og \textit{thinning} \fg{}.	\\
...
\begin{align*}
	\Pr&\left(\substack{\text{d'observer $n$ événements de $N_{1}$ avant}\\ \text{d'observer $m$ événement de $N_{2}$}}\right)
	=	\Pr(T_{1, n}	 < T_{2, m})	\\
	&=	\Pr\left(\substack{\text{d'observer au plus $\textcolor{teal}{m - 1}$ événements de $N_{2}$ avant}\\ \text{d'observer le $\textcolor{amethyst}{n}^{e}$ événement de $N_{1}$}}\right)	\\
	&=	\sum_{k = 0}^{\textcolor{teal}{m - 1}} \binom{\textcolor{amethyst}{n} + \textcolor{teal}{k - 1}}{n - 1} \left(\frac{\lambda_{1}}{\lambda_{1} + \lambda_{2}}\right)^{\textcolor{amethyst}{n}} \left(\frac{\lambda_{2}}{\lambda_{1} + \lambda_{2}}\right)^{\textcolor{teal}{k}}	\\
	&=	\Pr\left(\substack{\text{parmis les $n + m - 1$ premiers événements}\\ \text{au \textbf{moins} $\textcolor{amethyst}{n}$ proviennent de $N_{1}$ et}\\ \text{au \textbf{plus} $\textcolor{teal}{m - 1}$ proviennent de $N_{2}$}}\right)	
	=	\Pr\left(\substack{\text{$n^{e}$ événement de $N_{1}$ se produise avant}\\ \text{le $m^{e}$ événement de $N_{2}$}}\right)	\\
	&=	\sum_{k = \textcolor{amethyst}{n}}^{n + \textcolor{teal}{m - 1}} \binom{n + m - 1}{k} \left(\frac{\lambda_{1}}{\lambda_{1} + \lambda_{2}}\right)^{k} \left(\frac{\lambda_{2}}{\lambda_{1} + \lambda_{2}}\right)^{(n + m - 1) - k}	\\
\end{align*}
